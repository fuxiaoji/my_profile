...
function NoteViewerPage() {
  ...
  const headings = []; source.split(/\r?\n/).forEach(line => { const match = line.match(/^(#{1,4})\s+(.+)/); if (match) headings.push(match[2]) })
  const directory = path.slice(0, path.lastIndexOf('/') + 1); const encodedPath = path.split('/').map(encodeURIComponent).join('/'); const fileUrl = localMode ? `/local-note/${encodedPath}` : noteCdn(path); const assetBase = localMode ? `/local-note/${directory.split('/').map(encodeURIComponent).join('/')}` : noteCdn(directory)
